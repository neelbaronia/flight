...
export function liftCoefficient(angleDeg, flapBoost = 0) {
  const radians = (angleDeg * Math.PI) / 180
  const linear = 2 * Math.PI * radians + flapBoost
  const stallFade = Math.max(0.28, 1 - Math.max(0, Math.abs(angleDeg) - 15) * 0.075)
  return linear * stallFade
}

export function flightForces({ speed, area, angle, dragCoefficient = 0.035, flapBoost = 0, density = AIR_DENSITY }) {
  const dynamicPressure = 0.5 * density * speed ** 2
  const cl = liftCoefficient(angle, flapBoost)
