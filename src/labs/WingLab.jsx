...
function Airfoil({ angle }) {
  const geometry = useMemo(() => {
    const shape = new THREE.Shape()
    shape.moveTo(-2.4, 0)
    shape.bezierCurveTo(-1.2, 0.52, 0.85, 0.55, 2.3, 0.06)
    shape.bezierCurveTo(2.34, -0.13, 0.7, -0.2, -2.4, 0)
...
function DeflectedAirflow({ speed, cl, stalled }) {
...
function PairedPackets({ speed, stalled }) {
  ...
  useFrame((state) => {
    const cycle = (state.clock.elapsedTime * (0.045 + speed / 700)) % 1
...
function SurfacePressure({ liftPositive }) {
  const xPositions = [-1.35, 0, 1.35]
  const topLength = liftPositive ? 0.42 : 0.82
  const bottomLength = liftPositive ? 0.82 : 0.42
  return (
    <group>
      {xPositions.map((x) => <ForceArrow key={`top-${x}`} from={[x, 1.2, 1.58]} direction={[0, -1, 0]} length={topLength} color={liftPositive ? '#397f99' : '#f4c744'} />)}
      {xPositions.map((x) => <ForceArrow key={`bottom-${x}`} from={[x, -1.18, 1.58]} direction={[0, 1, 0]} length={bottomLength} color={liftPositive ? '#f4c744' : '#397f99'} />)}
      <Text position={[0, 1.5, 1.65]} fontSize={0.18} color="#245c70" outlineWidth={0.014} outlineColor="#fff9ef">
        {liftPositive ? 'LOW PRESSURE · SMALLER PUSH' : 'HIGH PRESSURE · BIGGER PUSH'}
      </Text>
      <Text position={[0, -1.42, 1.65]} fontSize={0.18} color="#775817" outlineWidth={0.014} outlineColor="#fff9ef">
...
function WingScene({ speed, angle, lift, drag, cl, stalled, viewMode }) {
  const liftLength = Math.min(3.1, 0.5 + Math.abs(lift) / 3500)
  ...
  return (
    <>
      ...
      <DeflectedAirflow speed={speed} cl={cl} stalled={stalled} />
      <Airfoil angle={angle} />
      {viewMode === 'pressure' && <SurfacePressure liftPositive={lift >= 0} />}
      {viewMode === 'packets' && <PairedPackets speed={speed} stalled={stalled} />}
      <ForceArrow from={[0, 0.48, 0]} direction={[0, lift >= 0 ? 1 : -1, 0]} length={liftLength} color="#e6543f" label={`NET LIFT · ${formatForce(lift)}`} />
      <ForceArrow from={[3.6, 0.62, 1.9]} direction={[-1, 0, 0]} length={dragLength} color="#304e54" />
      <Text position={[4.75, -2.15, 1.1]} fontSize={0.22} color="#285b60" outlineWidth={0.01} outlineColor="#f5eadb" anchorX="right">RELATIVE AIR ←</Text>
      <Text position={[0, -2.7, 1.4]} fontSize={0.21} color="#684d45" outlineWidth={0.012} outlineColor="#ead8b7">PAINTED GROUND MOVES LEFT ←</Text>
...
export function WingLab() {
  ...
  const area = 16
  const forces = flightForces({ speed, area, angle })
  const stalled = Math.abs(angle) > 15
    ...
    <div className="lab-layout lab-layout--painted-notes">
      <section className="demo-pane demo-pane--wing" aria-label="Interactive wing model">
        ...
        <div className="scene-mode" aria-label="Wing visualization mode">
          <button type="button" className={viewMode === 'packets' ? 'is-active' : ''} onClick={() => setViewMode('packets')}>Air packets</button>
        ...
        <Canvas camera={{ position: [7, 4, 7], fov: 42 }} shadows dpr={[1, 1.75]} gl={{ preserveDrawingBuffer: true }}>
          <WingScene speed={speed} angle={angle} lift={forces.lift} drag={forces.drag} cl={forces.cl} stalled={stalled} viewMode={viewMode} />
        </Canvas>
      ...
      <aside className="lesson-pane">
        ...
        <div className="metric-grid">
          ...
          <Metric label="Air pressure" value={`${forces.dynamicPressure.toFixed(0)} Pa`} tone="yellow" />
        </div>

        <section className="lesson-section">
          <h2>Where is the force made?</h2>
          <div className="pressure-explainer" aria-label="Pressure forces acting on a wing">
            <div className="pressure-explainer__top"><span>↓</span><span>↓</span><span>↓</span><b>Lower pressure above</b></div>
            <div className="pressure-explainer__wing">THE WING'S SKIN</div>
        ...
        <section className="lesson-section step-list">
          ...
          <Note>Switch to “Air packets” above the model. At positive lift, A advances ahead of B. Raise the angle past 15° and A instead peels away into the separated flow.</Note>
        </section>

        <section className="lesson-section">
          <h2>Why wings stall</h2>
          <p className="body-copy">Beyond roughly 15°, the pressure rise toward the rear becomes too steep for the slow air near the surface. The upper streamlines detach, a tumbling wake forms, drag jumps, and lift starts to fade. Raise the angle past 15° to see the packet path and white streamlines separate in the graphic.</p>
        </section>
      </aside>
    </div>
  )
